...
{
    public sealed class RequestBehaviorB<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
        where TRequest : IRequest<TResponse>
    {
        private readonly PipelineExecutionRecorder _recorder;

        public RequestBehaviorB(PipelineExecutionRecorder recorder)
        {
            _recorder = recorder;
        }
        ...
        {
            _recorder.Events.Add("RequestB.Before");

            TResponse response = await next();

            _recorder.Events.Add("RequestB.After");
