...
{
    public sealed class RecordingRequestBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
        where TRequest : IRequest<TResponse>
    {
        private readonly PipelineExecutionRecorder _recorder;

        public RecordingRequestBehavior(PipelineExecutionRecorder recorder)
        {
            _recorder = recorder;
        }
        ...
        {
            _recorder.Events.Add("Request.Before");

            TResponse response = await next();

            _recorder.Events.Add("Request.After");
